<script lang="ts">
	...
	import { Connect, wtStore } from "$lib/stores/wt.svelte";
    ...
    async function handleLogin(event: SubmitEvent & { currentTarget: EventTarget & HTMLFormElement}) {
        ...
        const code = await fetch(`http://${data.get("login-ip")}:${data.get("login-port")}/login?name=${data.get("name")}`, {
            method: 'GET'
        }).then((r) => {
            if (r.status !== 200) {
                return '';
        ...
        const con = await Connect(code, wtip, wtport);
